...
const TaxAmendmentLeadReport = () => {
  ...
  useEffect(() => {
    document.title = "Tax Amendment Lead Report - Occams Portal";
  ...
  const fetchLeads = async () => {
    ...
    try {
      console.log('Fetching Tax Amendment leads from API...');

      // Make the API request with proper headers
      const response = await axios.get('https://play.occamsadvisory.com/portal/wp-json/v1/leads?product_id=936', {
        headers: {
      ...
      if (response && response.data && response.data.data && Array.isArray(response.data.data)) {
        ...
        // Filter leads for Tax Amendment
        apiLeads = apiLeads.filter(lead => {
          ...
          return category.includes('tax amendment') ||
                 productType.includes('tax amendment') ||
                 leadGroup.includes('tax amendment') ||
                 category.includes('taxamendment') ||
                 productType.includes('taxamendment') ||
                 leadGroup.includes('taxamendment') ||
                 productId === '936' || productId === 936;
        });

        console.log('Filtered leads for Tax Amendment:', apiLeads);

        if (apiLeads.length > 0) {
          setLeads(apiLeads);
        } else {
          console.warn('No Tax Amendment leads found in API response, using fallback data');
          setLeads(generateFallbackLeads());
          setError('No Tax Amendment leads found in API response. Using sample data instead.');
        }
      } else {
        console.error('API response format unexpected:', response);
        // If API returns unexpected format, use fallback data
        setLeads(generateFallbackLeads());
        setError('API returned unexpected data format. Using sample Tax Amendment data instead.');
      }
    } catch (err) {
      console.error('Error fetching leads:', err);
      setError(`Failed to fetch Tax Amendment leads: ${err.message}. Using sample data instead.`);
      setLeads(generateFallbackLeads());
    } finally {
      setLoading(false);
    }
  ...
  const generateFallbackLeads = () => {
    ...
    for (let i = 1; i <= 100; i++) {
      ...
      dummyLeads.push({
        ...
        product_type: 'Tax Amendment',
        lead_group: 'Tax Amendment',
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `tax_amendment_lead_report_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const exportToPDF = () => {
    try {
      ...
      // Add title
      doc.setFontSize(16);
      doc.text('Tax Amendment Lead Report', 15, 15);
      ...
      doc.save(`tax_amendment_lead_report_${new Date().toISOString().slice(0, 10)}.pdf`);
    } catch (error) {
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Tax Amendment Leads');

      // Generate Excel file and trigger download
      XLSX.writeFile(wb, `tax_amendment_lead_report_${new Date().toISOString().slice(0, 10)}.xlsx`);
    } catch (error) {
              ...
              <div className="white_card_header">
                <div className="box_header m-0 new_report_header">
                  <div className="title_img">
                    ...
                    <h4 className="text-white">Tax Amendment Lead Report</h4>
                  </div>
                </div>
              </div>
                              ...
                              {allColumns.map(column => {
                                ...
                                switch (column.id) {
                                  ...
                                  case 'bookACall':
                                    ...
                                    const calendlyUrl = `https://calendly.com/occams-tax-amendment-experts/rmj?name=${businessName}&email=${email}&a1=${phone}`;
...
export default TaxAmendmentLeadReport;