...
const ProjectReport = ({ projectType = 'all' }) => {
  ...
  useEffect(() => {
    document.title = `${projectType.charAt(0).toUpperCase() + projectType.slice(1)} Projects - Occams Portal`;
  ...
  const fetchProjects = async () => {
    ...
    try {
      ...
      let apiUrl = 'https://play.occamsadvisory.com/portal/wp-json/productsplugin/v1/projects?product_id=935';
      ...
      if (response && response.data) {
        ...
        // If response.data is not an array but has a data property that is an array
          ...
          fallbackData.forEach(project => {
            ...
            // Don't modify the business_name directly
          });
  ...
  const generateFallbackProjects = () => {
    ...
    const products = ['ERC', 'STC', 'Audit Advisory'];
    const projects = ['None', 'Updated Comm Cal - ERC', 'Stu Bharat - STC', 'AA play qa - Audit Advisory', 'ERC play sp#12 - ERC'];
    const collaborators = ['Monitor app', 'Demo Finance', 'None'];
    const milestones = ['ERC Fulfillment', 'STC Enrollment', 'None'];
    const stages = ['ERC Fulfillment', 'STC Enrollment', 'None'];
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      const collaboratorIndex = Math.floor(Math.random() * collaborators.length);
      ...
      dummyProjects.push({
        id: `${1700 + i}`,
        date: formattedDate,
        business_name: businessNames[businessIndex],
  ...
  // Define all available columns with groups based on the image provided
  const columnGroups = [
    {
      id: 'basic',
      title: 'Basic Information',
      columns: [
        { id: 'projectId', label: 'ID', field: 'id', sortable: true },
        { id: 'date', label: 'Date', field: 'date', sortable: true },
        { id: 'businessName', label: 'Business Name', field: 'business_name', sortable: true },
        { id: 'contactCard', label: 'Contact Card', field: 'contact_card', sortable: false }
      ]
    },
    {
      id: 'project',
      ...
      columns: [
        ...
        { id: 'project', label: 'Project', field: 'project', sortable: true },
    ...
    },
    {
      ...
      columns: [
        { id: 'stage', label: 'Stage', field: 'stage', sortable: true },
  ...
  const defaultVisibleColumns = ['projectId', 'date', 'businessName', 'contactCard', 'product', 'project', 'collaborators', 'milestone', 'stage', 'taxNowSignupStatus', 'notes'];
  ...
  const filteredProjects = projects.filter(project => {
    ...
    const assignedTo = String(project.assigned_to || '').toLowerCase();
    const teamMembers = String(project.team_members || '').toLowerCase();
    ...
    const matchesSearch = searchTerm === '' ||
      ...
      client.includes(searchTermLower) ||
      type.includes(searchTermLower) ||
      assignedTo.includes(searchTermLower) ||
      teamMembers.includes(searchTermLower);
  ...
  const exportToCSV = () => {
    ...
    const csvData = filteredProjects.map(project => {
      return visibleColumnsData.map(column => {
        ...
        if (column.id === 'projectId') return project.id || '';
        if (column.id === 'date') return project.date || '';
        if (column.id === 'businessName') {
          const businessName = project.business_name || '';
          // Escape quotes in CSV
          return `"${businessName.replace(/"/g, '""')}"`;
        }
        if (column.id === 'product') return project.product || '';
        if (column.id === 'project') return project.project || '';
        if (column.id === 'collaborators') return project.collaborators || '';
        if (column.id === 'milestone') return project.milestone || '';
        if (column.id === 'stage') return project.stage || '';
    ...
    try {
      ...
      const tableRows = filteredProjects.map(project => {
        return visibleColumnsData.map(column => {
          ...
          if (column.id === 'projectId') return project.id || '';
          if (column.id === 'date') return project.date || '';
          if (column.id === 'businessName') return project.business_name || '';
          if (column.id === 'product') return project.product || '';
          if (column.id === 'project') return project.project || '';
          if (column.id === 'collaborators') return project.collaborators || '';
          if (column.id === 'milestone') return project.milestone || '';
    ...
    try {
      ...
      const excelData = filteredProjects.map(project => {
        ...
        visibleColumnsData.forEach(column => {
          // Handle special columns with custom rendering
          if (column.id === 'projectId') rowData[column.label] = project.id || '';
          else if (column.id === 'date') rowData[column.label] = project.date || '';
          else if (column.id === 'businessName') rowData[column.label] = project.business_name || '';
          else if (column.id === 'product') rowData[column.label] = project.product || '';
          else if (column.id === 'project') rowData[column.label] = project.project || '';
          ...
          else if (column.id === 'stage') rowData[column.label] = project.stage || '';
