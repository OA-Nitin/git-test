...
const TaxAmendmentProjectsReport = () => {
  // State for API data
  ...
  useEffect(() => {
    document.title = "Tax Amendment Projects Report - Occams Portal"; // Set title for Tax Amendment Projects Report page
  ...
  const fetchProjects = async () => {
    ...
    try {
      ...
      const apiUrl = 'https://play.occamsadvisory.com/portal/wp-json/portalapi/v1/projects?product_id=936';
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `tax_amendment_projects_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const exportToPDF = () => {
    try {
      ...
      // Add title and date
      const title = 'Tax Amendment Projects Report';
      ...
      doc.save(`tax_amendment_projects_report_${new Date().toISOString().slice(0, 10)}.pdf`);
    } catch (error) {
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Tax Amendment Projects');

      // Generate Excel file and trigger download
      XLSX.writeFile(wb, `tax_amendment_projects_${new Date().toISOString().slice(0, 10)}.xlsx`);
    } catch (error) {
              ...
              <div className="white_card_header">
                <div className="box_header m-0 new_report_header">
                  <div className="title_img">
                    ...
                    <h4 className="text-white">Tax Amendment Projects Report</h4>
                  </div>
                </div>
              </div>
...
export default TaxAmendmentProjectsReport;