...
const STCOpportunities = () => {
  ...
  useEffect(() => {
    document.title = "STC Opportunities - Occams Portal";
  ...
  const fetchOpportunities = async (limit = 100, retryAttempt = 0) => {
    ...
    try {
      console.log(`Fetching STC opportunities from API (attempt ${retryAttempt + 1} of ${maxRetries + 1})...`);

      // Use the direct API endpoint with product_id filter for STC (937)
      const apiEndpoint = 'https://play.occamsadvisory.com/portal/wp-json/productsplugin/v1/opportunities?product_id=937';

      console.log(`Requesting STC opportunities from API endpoint: ${apiEndpoint}`);
      ...
      if (apiOpportunities.length > 0) {
        ...
        console.log('No STC opportunities found in API response');
        setOpportunities([]);
        setError('No STC opportunities found in API response. Please try again later.');
      }

      return true; // Success
    } catch (err) {
      console.error(`Error fetching STC opportunities (attempt ${retryAttempt + 1}):`, err);
      ...
      if (err.name === 'AbortError' || err.code === 'ECONNABORTED' || err.name === 'CanceledError') {
        ...
        setError(`Failed to fetch STC opportunities: ${err.message}. Please try again later.`);
      }
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", "stc_opportunities.csv");
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "STC Opportunities");

    // Save workbook
    XLSX.writeFile(wb, "stc_opportunities.xlsx");
  };
  ...
  const exportToPDF = () => {
    ...
    // Set title
    doc.setFontSize(18);
    doc.text("STC Opportunities Report", 14, 22);
    ...
    // Save document
    doc.save("stc_opportunities.pdf");
  };
        ...
        <div className="col-12">
          <div className="card mb-4">
            <div className="card-header d-flex justify-content-between align-items-center">
              <h5 className="mb-0">STC Opportunities</h5>
            ...
            <div className="card-body">
              ...
              {loading ? (
                <div className="text-center py-5">
                  ...
                  <p className="text-muted">Loading STC opportunities...</p>
                </div>
              ) : error ? (
                ...
                <div className="alert alert-info">
                  ...
                  No STC opportunities found matching your criteria.
                </div>
              ) : (
...
export default STCOpportunities;
