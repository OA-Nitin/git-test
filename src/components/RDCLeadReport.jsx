...
const RDCLeadReport = () => {
  ...
  useEffect(() => {
    document.title = "RDC Lead Report - Occams Portal";
  ...
  const fetchLeads = async () => {
    ...
    try {
      console.log('Fetching RDC leads from API...');

      // Make the API request with proper headers
      const response = await axios.get('https://play.occamsadvisory.com/portal/wp-json/v1/leads?product_id=932', {
        headers: {
      ...
      if (response && response.data && response.data.data && Array.isArray(response.data.data)) {
        ...
        apiLeads = apiLeads.filter(lead => {
          ...
          return category.includes('rdc') ||
                 productType.includes('rdc') ||
                 leadGroup.includes('rdc') ||
                 productId === '932' || productId === 932;
        });

        console.log('Filtered leads for RDC:', apiLeads);

        if (apiLeads.length > 0) {
          setLeads(apiLeads);
        } else {
          console.warn('No RDC leads found in API response, using fallback data');
          setLeads(generateFallbackLeads());
          setError('No RDC leads found in API response. Using sample data instead.');
        }
      } else {
        console.error('API response format unexpected:', response);
        // If API returns unexpected format, use fallback data
        setLeads(generateFallbackLeads());
        setError('API returned unexpected data format. Using sample RDC data instead.');
      }
    } catch (err) {
      console.error('Error fetching leads:', err);
      setError(`Failed to fetch RDC leads: ${err.message}. Using sample data instead.`);
      setLeads(generateFallbackLeads());
    } finally {
      setLoading(false);
    }
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `rdc_lead_report_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const exportToPDF = () => {
    try {
      ...
      // Add title
      doc.setFontSize(16);
      doc.text('RDC Lead Report', 15, 15);
      ...
      doc.save(`rdc_lead_report_${new Date().toISOString().slice(0, 10)}.pdf`);
    } catch (error) {
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'RDC Leads');

      // Generate Excel file and trigger download
      XLSX.writeFile(wb, `rdc_lead_report_${new Date().toISOString().slice(0, 10)}.xlsx`);
    } catch (error) {
              ...
              <div className="white_card_header">
                <div className="box_header m-0 new_report_header">
                  <div className="title_img">
                    ...
                    <h4 className="text-white">RDC Lead Report</h4>
                  </div>
                </div>
              </div>
                              ...
                              {allColumns.map(column => {
                                ...
                                switch (column.id) {
                                  ...
                                  case 'bookACall':
                                    ...
                                    const calendlyUrl = `https://calendly.com/occams-rdc-experts/rmj?name=${businessName}&email=${email}&a1=${phone}`;
