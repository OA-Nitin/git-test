...
const AuditAdvisoryOpportunities = () => {
  // State for API data
  ...
  useEffect(() => {
    document.title = "Audit Advisory Opportunities - Occams Portal";
  ...
  const fetchOpportunities = async (limit = 100, retryAttempt = 0) => {
    ...
    try {
      console.log(`Fetching Audit Advisory opportunities from API (attempt ${retryAttempt + 1} of ${maxRetries + 1})...`);

      // Use the direct API endpoint with product_id filter for Audit Advisory (934)
      const apiEndpoint = 'https://play.occamsadvisory.com/portal/wp-json/productsplugin/v1/opportunities?product_id=934';

      console.log(`Requesting Audit Advisory opportunities from API endpoint: ${apiEndpoint}`);
      ...
      if (apiOpportunities.length > 0) {
        ...
        console.log('No Audit Advisory opportunities found in API response');
        setOpportunities([]);
        setError('No Audit Advisory opportunities found in API response. Please try again later.');
      }

      return true; // Success
    } catch (err) {
      console.error(`Error fetching Audit Advisory opportunities (attempt ${retryAttempt + 1}):`, err);
      ...
      if (err.name === 'AbortError' || err.code === 'ECONNABORTED' || err.name === 'CanceledError') {
        ...
        setError(`Failed to fetch Audit Advisory opportunities: ${err.message}. Please try again later.`);
      }
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", "audit_advisory_opportunities.csv");
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Audit Advisory Opportunities");

    // Save workbook
    XLSX.writeFile(wb, "audit_advisory_opportunities.xlsx");
  };
  ...
  const exportToPDF = () => {
    ...
    // Set title
    doc.setFontSize(18);
    doc.text("Audit Advisory Opportunities Report", 14, 22);
    ...
    // Save document
    doc.save("audit_advisory_opportunities.pdf");
  };
              ...
              <div className="white_card_header">
                <div className="box_header m-0 new_report_header">
                  <div className="title_img">
                    ...
                    <h4 className="text-white">Audit Advisory Opportunities</h4>
                  </div>
                </div>
              </div>
              <div className="white_card_body">
                ...
                {loading && (
                  <div className="text-center my-5">
                    ...
                    <p className="mt-2">Loading Audit Advisory opportunities data...</p>
                  </div>
                )}
...
export default AuditAdvisoryOpportunities;
