...
const PartnershipOpportunities = () => {
  ...
  useEffect(() => {
    document.title = "Partnership Opportunities - Occams Portal";
  ...
  const fetchOpportunities = async (limit = 100, retryAttempt = 0) => {
    ...
    try {
      console.log(`Fetching Partnership opportunities from API (attempt ${retryAttempt + 1} of ${maxRetries + 1})...`);

      // Use the direct API endpoint with product_id filter for Partnership (938)
      const apiEndpoint = 'https://play.occamsadvisory.com/portal/wp-json/productsplugin/v1/opportunities?product_id=938';

      console.log(`Requesting Partnership opportunities from API endpoint: ${apiEndpoint}`);
      ...
      if (apiOpportunities.length > 0) {
        ...
        console.log('No Partnership opportunities found in API response');
        setOpportunities([]);
        setError('No Partnership opportunities found in API response. Please try again later.');
      }

      return true; // Success
    } catch (err) {
      console.error(`Error fetching Partnership opportunities (attempt ${retryAttempt + 1}):`, err);
      ...
      if (err.name === 'AbortError' || err.code === 'ECONNABORTED' || err.name === 'CanceledError') {
        ...
        setError(`Failed to fetch Partnership opportunities: ${err.message}. Please try again later.`);
      }
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", "partnership_opportunities.csv");
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Partnership Opportunities");

    // Save workbook
    XLSX.writeFile(wb, "partnership_opportunities.xlsx");
  };
  ...
  const exportToPDF = () => {
    ...
    // Set title
    doc.setFontSize(18);
    doc.text("Partnership Opportunities Report", 14, 22);
    ...
    // Save document
    doc.save("partnership_opportunities.pdf");
  };
              ...
              <div className="white_card_header">
                <div className="box_header m-0 new_report_header">
                  <div className="title_img">
                    ...
                    <h4 className="text-white">Partnership Opportunities</h4>
                  </div>
                </div>
              </div>
              <div className="white_card_body">
                ...
                {loading && (
                  <div className="text-center my-5">
                    ...
                    <p className="mt-2">Loading Partnership opportunities data...</p>
                  </div>
                )}
