...
const AuditAdvisoryLeadReport = () => {
  ...
  useEffect(() => {
    document.title = "Audit Advisory Lead Report - Occams Portal";
  ...
  const fetchLeads = async () => {
    ...
    try {
      console.log('Fetching Audit Advisory leads from API...');

      // Make the API request with proper headers
      const response = await axios.get('https://play.occamsadvisory.com/portal/wp-json/v1/leads?product_id=934', {
        headers: {
      ...
      if (response && response.data && response.data.data && Array.isArray(response.data.data)) {
        ...
        // Filter leads for Audit Advisory
        apiLeads = apiLeads.filter(lead => {
          ...
          return category.includes('audit advisory') ||
                 productType.includes('audit advisory') ||
                 leadGroup.includes('audit advisory') ||
                 category.includes('auditadvisory') ||
                 productType.includes('auditadvisory') ||
                 leadGroup.includes('auditadvisory') ||
                 productId === '934' || productId === 934;
        });

        console.log('Filtered leads for Audit Advisory:', apiLeads);

        if (apiLeads.length > 0) {
          setLeads(apiLeads);
        } else {
          console.warn('No Audit Advisory leads found in API response, using fallback data');
          setLeads(generateFallbackLeads());
          setError('No Audit Advisory leads found in API response. Using sample data instead.');
        }
      } else {
        console.error('API response format unexpected:', response);
        // If API returns unexpected format, use fallback data
        setLeads(generateFallbackLeads());
        setError('API returned unexpected data format. Using sample Audit Advisory data instead.');
      }
    } catch (err) {
      console.error('Error fetching leads:', err);
      setError(`Failed to fetch Audit Advisory leads: ${err.message}. Using sample data instead.`);
      setLeads(generateFallbackLeads());
    } finally {
      setLoading(false);
    }
  ...
  const generateFallbackLeads = () => {
    ...
    for (let i = 1; i <= 100; i++) {
      ...
      dummyLeads.push({
        ...
        product_type: 'Audit Advisory',
        lead_group: 'Audit Advisory',
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `audit_advisory_lead_report_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const exportToPDF = () => {
    try {
      ...
      // Add title
      doc.setFontSize(16);
      doc.text('Audit Advisory Lead Report', 15, 15);
      ...
      doc.save(`audit_advisory_lead_report_${new Date().toISOString().slice(0, 10)}.pdf`);
    } catch (error) {
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Audit Advisory Leads');

      // Generate Excel file and trigger download
      XLSX.writeFile(wb, `audit_advisory_lead_report_${new Date().toISOString().slice(0, 10)}.xlsx`);
    } catch (error) {
              ...
              <div className="white_card_header">
                <div className="box_header m-0 new_report_header">
                  <div className="title_img">
                    ...
                    <h4 className="text-white">Audit Advisory Lead Report</h4>
                  </div>
                </div>
              </div>
                              ...
                              {allColumns.map(column => {
                                ...
                                switch (column.id) {
                                  ...
                                  case 'bookACall':
                                    ...
                                    const calendlyUrl = `https://calendly.com/occams-audit-advisory-experts/rmj?name=${businessName}&email=${email}&a1=${phone}`;
...
export default AuditAdvisoryLeadReport;