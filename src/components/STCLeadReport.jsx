...
const STCLeadReport = () => {
  ...
  useEffect(() => {
    document.title = "STC Lead Report - Occams Portal";
  ...
  const fetchLeads = async () => {
    ...
    try {
      console.log('Fetching STC leads from API...');

      // Make the API request with proper headers
      // Try without product_id filter first to get all leads
      const response = await axios.get('https://play.occamsadvisory.com/portal/wp-json/v1/leads?product_id=937', {
        headers: {
      ...
      console.log('API Response Structure:', {
        ...
        dataPropertyType: response.data && typeof response.data.data,
        isArray: response.data && response.data.data && Array.isArray(response.data.data),
        dataLength: response.data && response.data.data && Array.isArray(response.data.data) ? response.data.data.length : 'N/A'
      });
      ...
      if (response && response.data) {
        if (Array.isArray(response.data)) {
          ...
          apiLeads = response.data;
        } else if (response.data.data && Array.isArray(response.data.data)) {
          // If response.data has a data property that is an array
          console.log('Response.data.data is an array');
          apiLeads = response.data.data;
        } else if (typeof response.data === 'object') {
          // If response.data is an object, try to extract leads from it
          console.log('Response.data is an object, trying to extract leads');
          // Convert object to array if it's not already an array
          const possibleLeads = Object.values(response.data).filter(item =>
            item && typeof item === 'object' && (item.lead_id || item.id)
          );
          if (possibleLeads.length > 0) {
            console.log('Found leads in object properties');
            apiLeads = possibleLeads;
          }
        }
      }

      console.log('Extracted API Leads:', apiLeads);

      // Filter leads for STC if we have any leads
      if (apiLeads.length > 0) {
        const filteredLeads = apiLeads.filter(lead => {
          // Make sure lead is an object
          if (!lead || typeof lead !== 'object') {
            console.log('Skipping non-object lead:', lead);
            return false;
          }

          const category = String(lead.category || '').toLowerCase();
          const productType = String(lead.product_type || '').toLowerCase();
          const leadGroup = String(lead.lead_group || '').toLowerCase();
          const productId = lead.product_id;

          // Log each lead's filtering properties
          console.log('Lead filtering properties:', {
          ...
          return category.includes('stc') ||
                 productType.includes('stc') ||
                 leadGroup.includes('stc') ||
                 productId === '937' || productId === 937;
        });

        console.log('Filtered leads for STC:', filteredLeads);

        if (filteredLeads.length > 0) {
          setLeads(filteredLeads);
          setError(null); // Clear any previous error
        } else {
          console.warn('No STC leads found after filtering, using fallback data');
          setLeads(generateFallbackLeads());
          setError('No STC leads found in API response. Using sample data instead.');
        }
      } else {
        console.error('No leads found in API response:', response);
        // If no leads found, use fallback data
        setLeads(generateFallbackLeads());
        setError('No leads found in API response. Using sample STC data instead.');
      }
    } catch (err) {
      console.error('Error fetching leads:', err);
      setError(`Failed to fetch STC leads: ${err.message}. Using sample data instead.`);
      setLeads(generateFallbackLeads());
    } finally {
      setLoading(false);
    }
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `stc_lead_report_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const exportToPDF = () => {
    try {
      ...
      // Add title
      doc.setFontSize(16);
      doc.text('STC Lead Report', 15, 15);
      ...
      doc.save(`stc_lead_report_${new Date().toISOString().slice(0, 10)}.pdf`);
    } catch (error) {
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'STC Leads');

      // Generate Excel file and trigger download
      XLSX.writeFile(wb, `stc_lead_report_${new Date().toISOString().slice(0, 10)}.xlsx`);
    } catch (error) {
              ...
              <div className="white_card_header">
                <div className="box_header m-0 new_report_header">
                  <div className="title_img">
                    ...
                    <h4 className="text-white">STC Lead Report</h4>
                  </div>
                </div>
              </div>
                              ...
                              {allColumns.map(column => {
                                ...
                                switch (column.id) {
                                  ...
                                  case 'bookACall':
                                    ...
                                    const calendlyUrl = `https://calendly.com/occams-stc-experts/rmj?name=${businessName}&email=${email}&a1=${phone}`;
