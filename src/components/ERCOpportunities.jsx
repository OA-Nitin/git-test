...
const ERCOpportunities = () => {
  ...
  useEffect(() => {
    document.title = "ERC Opportunities - Occams Portal";
  ...
  const fetchOpportunities = async (limit = 100, retryAttempt = 0) => {
    ...
    try {
      console.log(`Fetching ERC opportunities from API (attempt ${retryAttempt + 1} of ${maxRetries + 1})...`);

      // Use the direct API endpoint with product_id filter for ERC (935)
      const apiEndpoint = 'https://play.occamsadvisory.com/portal/wp-json/productsplugin/v1/opportunities?product_id=935';

      console.log(`Requesting ERC opportunities from API endpoint: ${apiEndpoint}`);
      ...
      if (apiOpportunities.length > 0) {
        ...
        console.log('No ERC opportunities found in API response');
        setOpportunities([]);
        setError('No ERC opportunities found in API response. Please try again later.');
      }

      return true; // Success
    } catch (err) {
      console.error(`Error fetching ERC opportunities (attempt ${retryAttempt + 1}):`, err);
      ...
      if (err.name === 'AbortError' || err.code === 'ECONNABORTED' || err.name === 'CanceledError') {
        ...
        setError(`Failed to fetch ERC opportunities: ${err.message}. Please try again later.`);
      }
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", "erc_opportunities.csv");
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "ERC Opportunities");

    // Save workbook
    XLSX.writeFile(wb, "erc_opportunities.xlsx");
  };
  ...
  const exportToPDF = () => {
    ...
    // Set title
    doc.setFontSize(18);
    doc.text("ERC Opportunities Report", 14, 22);
    ...
    // Save document
    doc.save("erc_opportunities.pdf");
  };
        ...
        <div className="col-12">
          <div className="card mb-4">
            <div className="card-header d-flex justify-content-between align-items-center">
              <h5 className="mb-0">ERC Opportunities</h5>
            ...
            <div className="card-body">
              ...
              {loading ? (
                <div className="text-center py-5">
                  ...
                  <p className="text-muted">Loading ERC opportunities...</p>
                </div>
              ) : error ? (
                ...
                <div className="alert alert-info">
                  ...
                  No ERC opportunities found matching your criteria.
                </div>
              ) : (
...
export default ERCOpportunities;
